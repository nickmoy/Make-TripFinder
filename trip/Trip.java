...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        Road prevRoad = _map.getLabel(prev, cur);
        Road curRoad = null;
        double prevdist = prevRoad.length();
        while (iter.hasNext()) {
            prev = cur;
            cur = iter.next();
            curRoad = _map.getLabel(prev, cur);
            if (curRoad.toString().equals(prevRoad.toString())
                    && curRoad.direction()
                    .fullName().equals(prevRoad.direction().fullName())) {
                prevdist = prevdist + curRoad.length();
            } else {
                String log = seq + ". Take "
                        + prevRoad + " " + prevRoad.direction().fullName()
                        + " for " + String.format("%.1f", prevdist)
                        + " miles.";
                System.out.println(log);
                prevRoad = curRoad;
                prevdist = curRoad.length();
                seq++;
            }
        }
        String log = seq + ". Take "
